...
bool ata_write(uint32_t lba, uint8_t count, const void* buf) {
    if (!g_pri_master.present) return false;
    if (count == 0) count = 1;
    ...
    outb(ATA_PRI_LBA1, (uint8_t)(lba >> 8));
    outb(ATA_PRI_LBA2, (uint8_t)(lba >> 16));
    outb(ATA_PRI_COMMAND, ATA_CMD_WRITE_PIO);

    const uint16_t* in_buf = (const uint16_t*)buf;
    for (uint8_t s = 0; s < count; s++) {
        if (!wait_drq()) return false;
        // Per the ATA spec, host writes must give the drive a brief
        // moment between words; outw + a tight loop is fine in QEMU.
        for (int i = 0; i < 256; i++) outw(ATA_PRI_DATA, *in_buf++);
        io_wait_400ns();
    }

    // Cache flush so the writes are durable.
    if (!wait_not_busy()) return false;
    outb(ATA_PRI_COMMAND, ATA_CMD_FLUSH);
